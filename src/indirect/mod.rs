...
impl FfaIndirectMsg {
    ...
    pub fn write_indirect_msg(&self, base_addr: u64, seq_num: u16, _buf: &[u8]) -> FfaError {
        // Note if we have multi-threaded support we need to take a lock while accessing the queue
        unsafe {
            let asyncmsg = base_addr as *mut AsyncMsgHeader;
            let entry_count = (*asyncmsg).count as usize;
            let mut data_len = _buf.len();
            ...
            for b in &mut (*asyncmsg).bitmap {
                ...
                if state == QUEUE_STATE_FREE {
                    ...
                    ptr::copy_nonoverlapping(&_buf[data_index], data_base as *mut u8, copy_len);
                    data_index += copy_len;
                    data_len -= copy_len;
